...
def gaussian_smooth(img, ksize=5, sigma=5, padding='mirror', out_type=np.uint8):
    ...
    if is_odd:
        ...
        out1 = window[half] * _tmp[:, pad_size-half:-half]
        for i in range(1, half):
            out1 += window[i] * (_tmp[:, i-1:-pad_size+i-1] + _tmp[:, pad_size-i:-i])
        out1 += window[0] * _tmp[:, pad_size:]
        del _tmp

        res = window[half] * out1[pad_size-half:-half, :]
        for i in range(1, half):
            res += window[i] * (out1[i-1:-pad_size+i-1, :] + out1[pad_size-i:-i, :])
        res += window[0] * out1[pad_size:, :]
        del out1
...
def sobel_edge_detection(img, threshold=20, colormap='viridis', skip_colormap=False):
    ...
    gra_color = np.array(colors)[gra_index]
    gra_colormap = np.uint8(np.round(gra_color * mag_c[..., np.newaxis]))
...
if __name__ == '__main__':
    ...
    cv2.imwrite(out_dir + '%s_magnitude_k=10.png' % filename, cv2.cvtColor(mag_10, cv2.COLOR_GRAY2BGR))
